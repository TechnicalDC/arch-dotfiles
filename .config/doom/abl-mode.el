...
(defvar abl-keyword-list
  '("def" "define" "as" "extent" "if" "then" "else" "end" "do" "elseif"
    ...
    "run" "label" "no-box" "width" "where" "no-lock" "skip" "column" "inherits"
    "unformatted" "by" "buffer" "group" "view-as" "alert-box" "field"
    "init" "query" "next" "on-error" "avail" "begins" "no-error" "implements"
...
(define-abbrev-table 'abl-mode-abbrev-table
  (mapcar #'(lambda (v) (list v nil 1))
          (append abl-keyword-list abl-operator-list abl-type-list)))
...
(defun abl-backtab ()
  "Decrease the indentation of the current line by 3 spaces."
  (interactive)
  (save-excursion
    (beginning-of-line)
    (when (looking-at "   ")
...
(define-derived-mode abl-mode
  ...
  (set (make-local-variable 'tab-stop-list) (number-sequence 0 200 3)))

;;;###autoload
(add-to-list 'auto-mode-alist '("\\.\\(p\\|i\\|cls\\)\\'" . abl-mode))
